...
def _create_headless_driver():
    ...
    elif os.name != "nt" and os.path.exists("/etc/os-release"):
        # On Linux (Lightning AI, etc.), do NOT use webdriver-manager as it
        # downloads incompatible binaries.  Ask user to run setup.sh instead.
        raise RuntimeError(
            "Chrome/chromedriver not found on this Linux system. "
            "Please run:  bash setup.sh  in the terminal first."
        )
    else:
        # Fallback to webdriver-manager (local Mac dev, etc.)
